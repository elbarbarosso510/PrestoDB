...
inline std::unordered_map<ULONG64, ULONG64> functions;
void raw_read(ULONG64 address, void* buffer, ULONG64 size)
{
	if (IsBadReadPtr((PVOID)address, size)) return;
	memcpy(buffer, (PVOID)address, size);
}
#define MONOOFFSET(func, type, offset) type func { return read<type>( reinterpret_cast<ULONG64>( this ) + offset ); } 
...
inline std::string read_widechar(const ULONG64 address, const ULONG64 size) {
...
struct glist_t {
	MONOOFFSET(data(), ULONG64, 0x0)
		MONOOFFSET(next(), ULONG64, 0x8)
};

struct mono_root_domain_t {
	MONOOFFSET(domain_assemblies(), glist_t*, 0xC8)
		MONOOFFSET(domain_id(), int, 0xBC)
		MONOOFFSET(jitted_function_table(), ULONG64, 0x148)
};

struct mono_table_info_t {
	int get_rows() {
		return read<int>(reinterpret_cast<ULONG64>(this) + 0x8) & 0xFFFFFF;
	}
};

struct mono_method_t {
	std::string name() {
		auto name = read_widechar(read<ULONG64>(reinterpret_cast<ULONG64>(this) + 0x18), 128);
...
struct mono_class_field_t {
		MONOOFFSET(offset(), int, 0x18)
		std::string name() {
		auto name = read_widechar(read<ULONG64>(reinterpret_cast<ULONG64>(this) + 0x8), 128);
...
struct mono_class_runtime_info_t {
	MONOOFFSET(max_domain(), int, 0x0)
};

struct mono_vtable_t {
	MONOOFFSET(flags(), unsigned char, 0x30)

		ULONG64 get_static_field_data() {
		if ((this->flags() & 4) != 0)
			return read<ULONG64>(ULONG64(this) + 0x40 + 8 * read<int>(read<ULONG64>(ULONG64(this) + 0x0) + 0x5c));
...
struct mono_class_t {
	MONOOFFSET(num_fields(), int, 0x100)
		MONOOFFSET(runtime_info(), mono_class_runtime_info_t*, 0xd0)

		std::string name() {
		auto name = read_widechar(read<ULONG64>(reinterpret_cast<ULONG64>(this) + 0x48), 128);
		if (static_cast<std::uint8_t>(name[0]) == 0xEE) {
			char name_buff[32];
			sprintf_s(name_buff, 32, "\\u%04X", utf8_to_utf16(const_cast<char*>(name.c_str())));
			name = name_buff;
	...
	std::string namespace_name() {
		auto name = read_widechar(read<ULONG64>(reinterpret_cast<ULONG64>(this) + 0x50), 128);
		if (static_cast<std::uint8_t>(name[0]) == 0xEE) {
			char name_buff[32];
			sprintf_s(name_buff, 32, "\\u%04X", utf8_to_utf16(const_cast<char*>(name.c_str())));
			name = name_buff;
	...
	int get_num_methods() {
		const auto v2 = (read<int>(reinterpret_cast<ULONG64>(this) + 0x2a) & 7) - 1;
		switch (v2) {
		case 0:
		case 1:
			return read<int>(reinterpret_cast<ULONG64>(this) + 0xFC);
		...
		case 4u:
			return read<int>(reinterpret_cast<ULONG64>(this) + 0xF0);
	...
	mono_method_t* get_method(const int i) {
		return reinterpret_cast<mono_method_t*>(read<ULONG64>(read<ULONG64>(reinterpret_cast<ULONG64>(this) + 0xA0) + 0x8 * i));
	}

	mono_class_field_t* get_field(const int i) {
		return reinterpret_cast<mono_class_field_t*>(read<ULONG64>(reinterpret_cast<ULONG64>(this) + 0x98) + 0x20 * i);
	}
	...
	mono_vtable_t* get_vtable(mono_root_domain_t* domain) {
		...
		return reinterpret_cast<mono_vtable_t*>(read<ULONG64>(reinterpret_cast<ULONG64>(runtime_info) + 8 * domain_id + 8));
	}

	mono_method_t* find_method(const char* method_name) {
		auto mono_ptr = ULONG64();
		for (auto i = 0; i < this->get_num_methods(); i++) {
			...
			if (!strcmp(method->name().c_str(), method_name))
				mono_ptr = reinterpret_cast<ULONG64>(method);
		}
...
struct mono_hash_table_t {
	MONOOFFSET(size(), int, 0x18)
		MONOOFFSET(data(), ULONG64, 0x20)
		MONOOFFSET(next_value(), void*, 0x108)
		MONOOFFSET(key_extract(), unsigned int, 0x58)
...
struct mono_image_t {
	MONOOFFSET(flags(), int, 0x1C)

		mono_table_info_t* get_table_info(const int table_id) {
		if (table_id > 55)
			return nullptr;

		return reinterpret_cast<mono_table_info_t*>(reinterpret_cast<ULONG64>(this) + 0x10 * (static_cast<int>(table_id) + 0xE));
	}
...
struct mono_assembly_t {
	MONOOFFSET(mono_image(), mono_image_t*, 0x60)
};

namespace mono {
	inline mono_root_domain_t* get_root_domain() {
		return reinterpret_cast<mono_root_domain_t*>(mono_get_root_domain());
	}
	...
	inline void init_functions() { // credits: niceone1 (https://www.unknowncheats.me/forum/3434741-post11.html)
		...
		for (auto i = 0; i < read<int>(jitted_table + 0x8); i++) {
			const auto entry = read<ULONG64>(jitted_table + 0x10 + i * 0x8);
			if (!entry)
				continue;

			for (auto j = 0; j < read<int>(entry + 0x4); j++) {
				const auto function = read<ULONG64>(entry + 0x18 + j * 0x8);
				if (!function)
					continue;

				const auto mono_ptr = read<ULONG64>(function + 0x0);
				const auto jitted_ptr = read<ULONG64>(function + 0x10);
	...
	inline mono_assembly_t* domain_assembly_open(mono_root_domain_t* domain, const char* name) {
		...
		while (true) {
			...
			const auto data_name = read_widechar(read<ULONG64>(data + 0x10), 128);
